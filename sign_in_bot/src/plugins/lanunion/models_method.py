...
import asyncio
from .models import Sign, Trans , Final # 导入你的模型定义
    ...
    async def Export(cls, session: async_scoped_session,outpot_name) -> str:
        ...
        try:
            async with async_session() as session:
                # 异步执行查询
                result = await session.execute(text(f"SELECT * FROM Sign"))
...
class TansManger:
    ...
    async def is_database_empty(db_session):
        # 查询数据库，判断是否有数据
        result = await db_session.execute(text("SELECT 1 FROM Trans LIMIT 1"))
        return not result.fetchone()
...
class FinalManger:
    ...
    async def get_all_student_id(cls, session: async_scoped_session) -> set:
        """获取数据库中所有 student_id"""
        result = await session.execute(select(Final.student_id))
        return {row[0] for row in result}

    @classmethod
    async def get_Sign_by_student_id(cls, session: async_scoped_session, student_id: str) -> Optional[Final]:
        """根据 student_id 获取单个信息"""
        return await session.get(Final, student_id)

    @staticmethod
    async def is_database_empty(db_session):
        # 查询数据库，判断是否有数据
        result = await db_session.execute(text("SELECT 1 FROM Final LIMIT 1"))
        return not result.fetchone()

    @classmethod
    async def create_signmsg(cls, session: async_scoped_session, **kwargs) -> Final:
        """创建新的数据"""
        new_signmsg = Final(**kwargs)
        session.add(new_signmsg)
        await session.commit()
        return new_signmsg
    ...
    async def Export(cls, session: async_scoped_session,outpot_name) -> str:
        ...
        try:
            async with async_session() as session:
                # 异步执行查询
                result = await session.execute(text(f"SELECT * FROM Final"))
                data = result.mappings().all()
